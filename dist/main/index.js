...
async function main() {
    try {
        const cacheHit = await moveCache((0, getOptions_1.getOptions)());
        (0, core_1.setOutput)("cache-hit", cacheHit);
    }
    catch (error) {
        console.trace(error);
...
async function moveCache(options) {
    ...
    for (const target of cacheTargets) {
        if (await (0, io_util_1.exists)(target.targetPath)) {
            ...
            (0, core_1.debug)(`Cache restored: ${target.path}`);
            hitCache = true;
        }
        else {
            (0, core_1.debug)(`Cache miss: ${target.path}`);
        }
    }
    return hitCache;
}
