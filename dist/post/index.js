...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.buildTargetPaths = void 0;
const glob = __importStar(__nccwpck_require__(8090));
const path_1 = __nccwpck_require__(1017);
async function buildTargetPaths(targetDir, distDir, paths) {
    const globPaths = paths.map((x) => (0, path_1.join)(targetDir, x)).join("\n");
    ...
    return matchedPaths.map((path) => {
        const relativePath = (0, path_1.relative)(targetDir, path);
        return {
            path: relativePath,
            distPath: (0, path_1.resolve)(distDir, relativePath),
            targetPath: (0, path_1.resolve)(targetDir, relativePath)
        };
    });
}
exports.buildTargetPaths = buildTargetPaths;
...
const getOptions_1 = __nccwpck_require__(9420);
const pathBuilder_1 = __nccwpck_require__(8586);
async function post() {
    try {
        const options = (0, getOptions_1.getOptions)();
        const cacheTargets = await (0, pathBuilder_1.buildTargetPaths)(options.workingDir, options.cacheDir, options.paths);
        await (0, io_1.mkdirP)(options.cacheDir);
        for (const target of cacheTargets) {
            if (await (0, io_util_1.exists)(target.targetPath)) {
                await (0, io_1.mv)(target.targetPath, target.distPath, { force: true });
                (0, core_1.debug)(`Caching: ${target.targetPath}`);
            }
            else {
                (0, core_1.debug)(`Skipping: no matches found for ${target.targetPath}`);
            }
        }
        if (options.remoteDir !== null && !(await (0, io_util_1.exists)(options.remoteDir))) {
            await (0, io_1.cp)(options.cacheDir, options.remoteDir);
        }
    }
    catch (error) {
        console.trace(error);
